...
{
	/* Lab4: use the CCS runtime putchar so printf reaches the CIO console.
	Do not replace it with the dummy putchar macro above. */
...
static int tiny_print( char **out, const char *format, va_list args, unsigned int buflen )
{
	...
	for (; *format != 0; ++format) {
		if (*format == '%') {
			...
			if( *format == 's' ) {
				register const char *s = va_arg( args, const char * );
