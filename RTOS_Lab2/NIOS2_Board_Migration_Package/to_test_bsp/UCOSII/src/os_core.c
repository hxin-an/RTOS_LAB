...
static  void  OS_EDF_FindHighRdy(void);
        ...
        if (OSIntNesting == 0) {                           /* Reschedule only if all ISRs complete ... */
            if (OSLockNesting == 0) {                      /* ... and not locked.                      */
                ...
                if (OSPrioHighRdy != OSPrioCur) {          /* No Ctx Sw if current task is highest rdy */
                    ...
                    /* ==================== LAB1 CORE: PREEMPT EVENT TRACE ==================== */
                    /* LAB1: log preemption edges for final scheduling trace output. */
                    LogEvent(OSTime, EVENT_PREEMPT, OSPrioCur, OSPrioHighRdy);
                    /* LAB1: stop simulation after deadline miss to avoid extra trace noise. */
...
void  OSStart (void)
{
    if (OSRunning == OS_FALSE) {
        OS_EDF_FindHighRdy();                        /* Find earliest-deadline ready task             */
...
void  OSTimeTick (void)
{
    ...
    if (OSRunning == OS_TRUE) {
        OS_ENTER_CRITICAL();
        /* ==================== LAB1 CORE: RUNTIME BUDGET CONSUMPTION ==================== */
        /* LAB1: consume one computation-time unit from the currently running periodic task. */
        if ((OSTCBCur->OSTCBIsPeriodic == OS_TRUE) && (OSTCBCur->OSTCBCompTime > 0u)) {
        ...
        while (ptcb->OSTCBPrio != OS_TASK_IDLE_PRIO) {     /* Go through all TCBs in TCB list              */
            OS_ENTER_CRITICAL();
            /* ==================== LAB1 CORE: PERIOD RELEASE + DEADLINE CHECK ==================== */
            /* LAB1: periodic release/deadline check; reset budget and make task ready each period. */
            if ((ptcb->OSTCBIsPeriodic == OS_TRUE) && (ptcb->OSTCBPeriod > 0u)) {
                if ((OSTime % (INT32U)ptcb->OSTCBPeriod) == 0u) {
                    if ((SimDone == OS_FALSE) && (ptcb->OSTCBCompTime > 0u)) {
                        DeadlineFlag     = OS_TRUE;
                        DeadlineTime     = OSTime;
                        DeadlineTaskPrio = ptcb->OSTCBPrio;
                    }
                    ptcb->OSTCBCompTime = ptcb->OSTCBCompTimeMax;
...
#endif
#endif
    if (OSTCBPrioTbl[OS_TASK_IDLE_PRIO] != (OS_TCB *)0) {
        OSTCBPrioTbl[OS_TASK_IDLE_PRIO]->OSTCBDeadline = 0xFFFFFFFFu;
    }
}
/*$PAGE*/
/*
...
void  OS_Sched (void)
{
    ...
    if (OSIntNesting == 0) {                           /* Schedule only if all ISRs done and ...       */
        if (OSLockNesting == 0) {                      /* ... scheduler is not locked                  */
            ...
            if (OSPrioHighRdy != OSPrioCur) {          /* No Ctx Sw if current task is highest rdy     */
                OSTCBHighRdy = OSTCBPrioTbl[OSPrioHighRdy];
                /* ==================== LAB1 CORE: COMPLETE EVENT TRACE ==================== */
                /* LAB1: log completion edges for required Complete event sequence. */
                LogEvent(OSTime, EVENT_COMPLETE, OSPrioCur, OSPrioHighRdy);
#if OS_TASK_PROFILE_EN > 0
                OSTCBHighRdy->OSTCBCtxSwCtr++;         /* Inc. # of context switches to this task      */
#endif
...
static  void  OS_EDF_FindHighRdy (void)
{
    ...
    ptcb         = OSTCBList;
    best_tcb     = (OS_TCB *)0;
    min_deadline = 0xFFFFFFFFu;

    while (ptcb != (OS_TCB *)0) {
        if ((ptcb->OSTCBStat == OS_STAT_RDY) &&
            ((OSRdyTbl[ptcb->OSTCBY] & ptcb->OSTCBBitX) != 0u)) {
    ...
    if (best_tcb != (OS_TCB *)0) {
        OSPrioHighRdy = best_tcb->OSTCBPrio;
    } else {
...
INT8U  OS_TCBInit (INT8U prio, OS_STK *ptos, OS_STK *pbos, INT16U id, INT32U stk_size, void *pext, INT16U opt)
{
    ...
    /* ==================== LAB1 CORE: TCB EXTENSION INIT ==================== */
    /* LAB1: initialize periodic-task extensions to safe defaults for every new TCB. */
    ptcb->OSTCBCompTime      = 0u;
    ptcb->OSTCBCompTimeMax   = 0u;
    ptcb->OSTCBPeriod        = 0u;
    ptcb->OSTCBDeadline      = 0u;
    ptcb->OSTCBIsPeriodic    = OS_FALSE;
