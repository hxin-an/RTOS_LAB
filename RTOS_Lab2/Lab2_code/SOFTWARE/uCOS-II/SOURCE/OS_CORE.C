...
void  OS_EDF_FindHighRdy(void);   /* Forward declaration */
/*---------------------------------------------*/
...
void  OSIntExit (void)
{
    ...
    if (OSRunning == TRUE) {
        ...
        if ((OSIntNesting == 0) && (OSLockNesting == 0)) { /* Reschedule only if all ISRs complete ... */
            /*----- Lab 2: EDF Scheduler Modification -----*/
            OS_EDF_FindHighRdy();                          /* EDF: Find highest priority task ready    */
            /*---------------------------------------------*/
            if (OSPrioHighRdy != OSPrioCur) {              /* No Ctx Sw if current task is highest rdy */
                OSTCBHighRdy  = OSTCBPrioTbl[OSPrioHighRdy];
                /*----- Lab2: Log preemption event -----*/
...
#endif
    if (OSRunning == TRUE) {
        /*----- Lab2/Lab1 periodic model: decrement compTime of running task -----*/
        OS_ENTER_CRITICAL();
        if (OSTCBCur->OSTCBIsPeriodic == TRUE && OSTCBCur->OSTCBCompTime > 0) {
        ...
        while (ptcb->OSTCBPrio != OS_IDLE_PRIO) {          /* Go through all TCBs in TCB list          */
            OS_ENTER_CRITICAL();
            /*----- Lab2/Lab1 periodic model: check period boundary -----*/
            if (ptcb->OSTCBIsPeriodic == TRUE && ptcb->OSTCBPeriod > 0) {
                if ((OSTime % (INT32U)ptcb->OSTCBPeriod) == 0) {
...
#endif
    /*----- Lab 2: EDF Scheduler Modification -----*/
    OSTCBPrioTbl[OS_IDLE_PRIO]->OSTCBDeadline = 0xFFFFFFFF;    /* EDF: Idle task has max deadline */
    /*---------------------------------------------*/
}
/*$PAGE*/
/*
...
*
* Description: Finds the ready task with the earliest deadline and updates OSPrioHighRdy.
*********************************************************************************************************
*/
void OS_EDF_FindHighRdy (void)
{
    ...
    while (ptcb != (OS_TCB *)0) {
        if (ptcb->OSTCBStat == OS_STAT_RDY &&
            (OSRdyTbl[ptcb->OSTCBY] & ptcb->OSTCBBitX) != 0) { /* Must also be in ready bitmap */
            if (ptcb->OSTCBDeadline < min_deadline) {           /* First-found wins on tie     */
    ...
    if (best_tcb != (OS_TCB *)0) {
        ...
        OSPrioHighRdy = OS_IDLE_PRIO; /* Should never happen since idle task is always ready */
    }
}
/*---------------------------------------------*/
...
INT8U  OS_TCBInit (INT8U prio, OS_STK *ptos, OS_STK *pbos, INT16U id, INT32U stk_size, void *pext, INT16U opt)
{
    ...
    if (ptcb != (OS_TCB *)0) {
        ...
        ptcb->OSTCBCompTime    = 0;                        /* Init periodic task fields                 */
        ptcb->OSTCBCompTimeMax = 0;
        ptcb->OSTCBPeriod      = 0;
        /*----- Lab 2: EDF Scheduler Modification -----*/
        ptcb->OSTCBDeadline    = 0;                        /* EDF: Init deadline                        */
        /*---------------------------------------------*/
        ptcb->OSTCBIsPeriodic  = FALSE;
